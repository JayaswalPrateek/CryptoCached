...
below i have specified how each function can be called from the backend,
...
now i will specify the particular order in which the functions should be called
...
but we also need to demonstrate caching, so after calling backendObj.dbHandler() once then call backendObj.test(2)
...
# returns a tuple of results of fetchRates() for the entire week uses API for the date not in cache
backendObj.ratesThisWeek()  # used internally and should not be called from the frontend

backendObj.dbHandler()  # calls ratesThisWeek() internally and inserts it into the database
